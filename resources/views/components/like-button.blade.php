<div class="flex">
    <form method="POST" action="/tweets/{{ $tweet->id }}/like">
        @csrf
        <button type="submit" class="text-lg flex items-center justify-evenly mr-4">
            <div
                class="flex mr-1 items-center {{ $tweet->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }}">
    ...
    <form method="POST" action="/tweets/{{ $tweet->id }}/like">
        @csrf
        @method('DELETE')
        <button type="submit" class="text-lg flex items-center justify-evenly">
            <div
